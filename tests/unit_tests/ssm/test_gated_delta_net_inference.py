...
Tests:
  * ``ssm_state_shapes_per_request`` and ``MambaInferenceStateConfig.from_model``
...
def _make_prefill_context(gdn, batch_indices, cu_seqlens, token_count):
    """Build a minimal MambaMetadata + context for ``_ssm_prefill``."""
    device = torch.cuda.current_device()
    metadata = MambaMetadata(
        max_requests=int(batch_indices.numel()),
        max_tokens=token_count,
        mamba_chunk_size=gdn.chunk_size,
    ...
    return types.SimpleNamespace(mamba_metadata=metadata, mamba_slot_allocator=None)
...
class TestGatedDeltaNetInference:
    ...
    def test_from_model_recognizes_gdn(self):
        """MambaInferenceStateConfig.from_model must detect a GDN hybrid model and read
        its per-request shapes + chunk size (mixer lives in the self_attention slot)."""
        gdn = self.gdn

        # Mirror what HybridStack exposes: layer_type_list + layers whose GDN entry
        # is a TransformerLayer with the mixer in ``self_attention``.
        layer = types.SimpleNamespace(self_attention=gdn)
        decoder = types.SimpleNamespace(
            layer_type_list=[Symbols.GDN],
            layers=[layer],
            mamba_state_shapes_per_request=gdn.ssm_state_shapes_per_request,
        )
        model = types.SimpleNamespace(decoder=decoder, config=self.config)

        cfg = MambaInferenceStateConfig.from_model(model)
        ...
        assert cfg.ssm_states_shape == (
            gdn.num_v_heads_local_tp,
            gdn.key_head_dim,
            gdn.value_head_dim,
        )
        assert cfg.mamba_chunk_size == gdn.chunk_size == 64
